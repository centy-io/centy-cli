...
2. Shows stack trace that's not useful to end users
3. Contradictory message: "Daemon process started but is not responding" when it actually failed to start
4. No actionable guidance on how to fix the issue
...
2. Provide actionable suggestions based on error type:
   - ENOENT: Install instructions
   - EACCES: Permission fix instructions
   - Other: Generic message with debugging hints

3. Optionally list the paths that were searched (from `findDaemonBinary.ts`):
