...
import { daemonCreateItem } from './daemon-create-item.js'
...
describe('daemonCreateItem', () => {
  ...
  it('should resolve with response on success', async () => {
    ...
    const mockClient = {
      createItem: vi.fn((_req, _options, callback) => {
    ...
    const result = await daemonCreateItem({} as never)
  ...
  it('should reject with error on failure', async () => {
    ...
    await expect(daemonCreateItem({} as never)).rejects.toThrow('gRPC error')
  })
})
