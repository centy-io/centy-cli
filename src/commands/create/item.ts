...
export default class CreateItem extends Command {
  // eslint-disable-next-line no-restricted-syntax
  static override args = {
    type: Args.string({
      description:
        'Item type (singular name, e.g., issue, doc, epic, or custom type)',
      required: true,
    }),
  }
  ...
  static override description =
    'Create a new item of any registered type (issues, docs, epics, or custom types)'

  // eslint-disable-next-line no-restricted-syntax
  static override examples = [
    '<%= config.bin %> create item issue --title "Bug in login" --priority 1',
    '<%= config.bin %> create item doc --title "Getting Started" --body "# Guide"',
    '<%= config.bin %> create item epic --title "Auth overhaul" --status open --priority 1',
    '<%= config.bin %> create item bug --title "Crash on startup"',
    '<%= config.bin %> create item task --title "Review PR" --custom-field "assignee=alice"',
  ]
  ...
  static override flags = {
    ...
    body: Flags.string({
      char: 'b',
      description: 'Item body (markdown content)',
      default: '',
    }),
    status: Flags.string({
      char: 's',
  ...
  public async run(): Promise<void> {
    const { args, flags } = await this.parse(CreateItem)
